...
CREATE TABLE `tb_shop_auth_map` (
  ...
  KEY `fk_shop_auth_map_employee` (`employee_id`,`shop_id`),
...
```
2.上传到liunx服务器
3.创建执行脚本，可以后台执行vim startup.sh
```
nohup java -jar /home/pc20131001/o2o.war &
```
4.sh startup.sh //以守护线程的模式在后台执行
5. ps -ef|grep java可以查看是否在后台运行